...
impl Repository {
    pub async fn create_job(&self, db: &Pool<Postgres>, job: &Job) -> Result<(), Error> {
        ...
        {
            Err(err) => {
                error!("create_job: Inserting job failed: {}", &err);
                Err(err.into())
            }
            Ok(_) => Ok(()),
        }
    ...
    pub async fn update_job(&self, db: &Pool<Postgres>, job: &Job) -> Result<(), Error> {
        ...
        {
            Err(err) => {
                error!("update_job: updating job failed: {}", &err);
                Err(err.into())
            }
            Ok(_) => Ok(()),
        }
    ...
    pub async fn find_job_by_id(&self, db: &Pool<Postgres>, job_id: &Uuid) -> Result<Job, Error> {
        ...
        {
            Err(err) => {
                error!("find_job_by_id: finding job failed: {}", &err);
                Err(err.into())
            }
    ...
    pub async fn find_job_for_agent(
            ...
            .await
        {
            Err(err) => {
                error!(
                    "find_job_where_output_is_null: finding job failed: {}",
                    &err
                );
                Err(err.into())
            }
    ...
    pub async fn find_all_jobs(&self, db: &Pool<Postgres>) -> Result<Vec<Job>, Error> {
        ...
        match sqlx::query_as::<_, Job>(QUERY).fetch_all(db).await {
            Err(err) => {
                error!("find_all_jobs: finding jobs failed: {}", &err);
                Err(err.into())
            }
            Ok(res) => Ok(res),
        }
